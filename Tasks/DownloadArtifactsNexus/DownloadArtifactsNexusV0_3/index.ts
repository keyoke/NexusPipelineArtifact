...
async function run() {
  ...
  try {
    ...
    const hostUri: URL | undefined = new URL(
      tl.getEndpointUrl(connection, false)
    );
    ...
    const MAVEN_REPOSITORY_ASSET_FILENAMES: string = tl.getVariable(
    ...
    if (MAVEN_REPOSITORY_ASSET_FILENAMES) {
      ...
      if (MAVEN_REPOSITORY_ASSET_FILENAME.length == 1) {
        tl.setVariable(
          ...
          MAVEN_REPOSITORY_ASSET_FILENAME[0],
          false
        );
      }
    }
