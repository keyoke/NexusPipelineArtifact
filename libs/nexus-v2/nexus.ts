...
export class nexus {
  public async downloadAsset(
    ...
    let requestPath = '/service/local/artifact/maven/redirect';
  ...
  public async downloadAssets(
    ...
    const hostUri = new URL(nexusUrl);
    let lucenePath = '/service/local/lucene/search';
    ...
    const extensions: string[] = await this.parseExtensions(responseContent);
      ...
      xml2js.parseString(xml, { explicitArray: false }, function (err, result) {
        if (err) {
          ...
          if (result.searchNGResponse.totalCount == 1) {
            resolve(
              xpath.find(
                result.searchNGResponse.data.artifact.artifactHits,
                '//extension'
              )
            );
          } else {
            const message = 'Search result XML contains multiple artifactHits.';
            console.log(message);
            reject(message);
          }
        }
